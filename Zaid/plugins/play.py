...
fotoplay = ""
ngantri = "https://telegra.ph/file/2f6762e96eab0a1ef3644.jpg"
...
btnn =[
    [Button.url("💁 كروب الدعم", url=f"t.me/{Config.SUPPORT}"), Button.url("Cʜᴀɴɴᴇʟ 🙋", url=f"t.me/{Config.CHANNEL}")],
    [Button.inline("إغلاق 🗑️", data="cls")]]


#play
@Zaid.on(events.NewMessage(pattern="^[?!/]تشغيل"))
    ...
    if event.is_group:
        try:
            ...
            if hm == 0:
                ...
                caption = f"💡 ** تمت إضافة الأغنية إلى قائمة الانتظار »**` # {pos} `\ n \ n ** 🏷 الاسم: ** [{songname}] ({url}) \ n ** ⏱ Duration: **` {duration} `\ n🎧 ** مقدم الطلب: ** {from_user}"
                await botman.delete()
                ...
                try:
                    ...
                    caption = f"🏷 ** الاسم: ** [{songname}] ({url}) \ n ** ⏱ Duration: ** `{duration}` \ n💡 ** الحالة: ** `قيد التشغيل` \ n🎧 ** مقدم الطلب: ** {from_user}"
                    await botman.delete()
                    await event.client.send_file(chat_id, thumb, caption=caption, buttons=btnn)
        ...
        if chat_id in QUEUE:
            ...
            caption = f"💡 ** تمت إضافة الأغنية إلى قائمة الانتظار »**` # {pos} `\ n \ n ** 🏷 Title: ** [{songname}] ({link}) \ n ** 👥 معرف الدردشة: **` {chat_id } `\ n🎧 ** مقدم الطلب: ** {from_user}"
            ...
            try:
                ...
                caption = f"🏷 ** Title: ** [{songname}] ({link}) \ n ** 👥 معرّف الدردشة: ** `{chat_id}` \ n💡 ** الحالة: ** `قيد التشغيل` \ n🎧 ** مطلوب :** {from_user}"
...
@Zaid.on(events.NewMessage(pattern="^[?!/فيديو]"))
async def vplay(event):
    ...
    if replied and not replied.video and not replied.document:
        xnxx = await event.reply("`البحث في تفاصيل الفيديو ...`")
        query = event.text.split(maxsplit=1)[1]
        search = ytsearch(query)
        RESOLUSI = 720
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                💡 ** Video Streaming In Queue »**` # {pos} `\ n \ n ** 🏷 Title: ** [{songname}] ({url}) \ n ** ⏱ Duration: **` {duration} `\ n🎧 ** مطلوب: ** {from_user}"
                await xnxx.delete()
                await event.client.send_file(chat_id, thumb, caption=caption, buttons=btnn)
            else:
                try:
                    ...
                    await event.client.send_file(event.chat_id,
                        f"**🏷 ** __ بدأ بث الفيديو __ **: ** [{songname}] ({url}) \ n ** ⏱ Duration: ** `{duration}` \ n💡 ** الحالة: ** `قيد التشغيل` \ n🎧 ** مطلوب: ** {from_user}، أزرار = btnn "،
                        link_preview=False,
                    )
                except Exception as ep:
                    clear_queue(chat_id)
        ...
        xnxx = await event.reply("📥 ** تنزيل الملف الذي تم الرد عليه **")
        dl = await replied.download_media()
        link = f"https://t.me/c/{chat.id}/{event.reply_to_msg_id}"
        ...
        if chat_id in QUEUE:
            ...
            caption = f"💡 ** بدأ بث الفيديو »**` # {pos} `\ n \ n ** 🏷 title: ** [{songname}] ({link}) \ n ** 👥 معرف الدردشة: **` {chat_id} `\ n🎧 ** مطلوب: ** {from_user}"
            await event.client.send_file(chat_id, ngantri, caption=caption, buttons=btnn)
            await xnxx.delete()
        else:
            if RESOLUSI == 360:
            ...
            try:
                ...
                caption = f"🏷 ** title: ** [{songname}] ({link}) \ n ** 👥 معرف الدردشة: ** `{chat_id}` \ n💡 ** الحالة: ** `قيد التشغيل` \ n🎧 ** مطلوب :** {from_user}"
                await xnxx.delete()
                await event.client.send_file(chat_id, fotoplay, caption=caption, buttons=btnn)
            except Exception as ep:
                clear_queue(chat_id)
        ...
        if search == 0:
            ...
            if hm == 0:
                ...
                pos = add_to_queue(
                    chat_id, songname, ytlink, url, "Video", RESOLUSI)
                💡 ** تمت إضافة تدفق الفيديو في قائمة الانتظار »**` # {pos} `\ n \ n🏷 ** title: ** [{songname}] ({url}) \ n ** ⏱ Duration: **` {duration } `\ n🎧 ** مطلوب: ** {from_user}"
                await xnxx.delete()
                await event.client.send_file(chat_id, thumb, caption=caption, buttons=btnn)
            else:
                try:
                    ...
                    add_to_queue(
                        ...
                        "Video",
                        RESOLUSI)
                    🏷 ** Title: ** [{songname}] ({url}) \ n ** ⏱ Duration: ** `{duration}` \ n💡 ** الحالة: ** `قيد التشغيل` \ n🎧 ** مطلوب: ** {from_user}"
                    await xnxx.delete()
                    await event.client.send_file(chat_id, thumb, caption=caption, buttons=btnn)
                except Exception as ep:
                    clear_queue(chat_id)
...
async def vc_skip(event, perm):
    ...
    if len(event.text.split()) < 2:
        ...
        if op == 0:
            ...
            await event.reply(
                f"**⏭ تم تخطي ** \ n ** 🎧 قيد التشغيل الآن ** - [{op [0]}] ({op [1]}) "،
                link_preview=False,
            )
    else:
