...
MISC_HELP = """
** ✘ وحدة "احتمالات ونهايات" للأوامر الصغيرة والبسيطة التي لا تناسب أي مكان. ** ‣ `؟ id` - للحصول على معرف الدردشة الحالي أو معرف المستخدم الذي تم الرد عليه. ‣ `؟ info` - للحصول على معلومات المستخدم.
"""

@Zaid.on(events.NewMessage(pattern="^[!?/]ايدي"))
async def id(event):
...
@Zaid.on(events.NewMessage(pattern="^[!?/]معلومات ?(.*)"))
